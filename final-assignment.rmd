...
kable_summary_table <- summary_table %>%
  kable("latex", booktabs = TRUE, caption = "Summary Statistics for Each Type of Toilet") %>%
...
rownames(anova_table) <- c("area_code", "Residuals", "R-squared")

# # Display the table using kable
kable(anova_table, digits = 4, align = "c", row.names = TRUE)
...
kable_pairwise_table <- pairwise_table %>%
  kable("latex", booktabs = TRUE, caption = "Pairwise T-Test Results") %>%
  kable_styling(latex_options = c("striped", "scale_down"))
kable_pairwise_table
```
## Comparison Plot
...
fviz_nbclust(SLC_2007.Subset200_filtered, FUN = hcut, method = "wss")
fviz_nbclust(SLC_2007.Subset200_filtered, FUN = hcut, method = "silhouette")
...
clusterLabs <- cutree(hc, k = 3)  # Replace '4' with the optimal number of clusters found
...
# justification, when we use 3 there is a higer water consumption pattern.  compared to 2 clusters that potentially group 2 groups together.
...
SLC_2007.Subset_Index <- SLC_2007.Subset %>%
  select(kitchen_shared,toilet_shared,water_source_shared,water_meter,per_cap_con_all )
...
SLC_2007.Subset_Index <- SLC_2007.Subset_Index %>%
  mutate(shared_facilities_sum = kitchen_shared_recode +
    ...
    # water_source_shared_recode +  #  were negatively correlated with the total scale
    water_meter_recode)
